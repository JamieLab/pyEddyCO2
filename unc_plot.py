...
gs = GridSpec(1,2, figure=fig, wspace=0.25,hspace=0.15,bottom=0.07,top=0.95,left=0.075,right=0.95)
...
for i in range(combined.shape[1]):
    ...
    for j in range(combined.shape[0]):
        if i == 1:
            p = ax[0].bar(i,(combined[j,i]/totals[i])*100,bottom=bottom,color=cols[j],label=label[j])
        else:
            p = ax[0].bar(i,(combined[j,i]/totals[i])*100,bottom=bottom,color=cols[j])
        bottom = bottom + (combined[j,i]/totals[i])*100

print(totals)

for i in range(combined.shape[0]):
    ax[1].plot(np.array(range(0,combined.shape[1])),combined[i,:],color=cols[i],label=label[i])
ax[1].legend()
plt.show()
